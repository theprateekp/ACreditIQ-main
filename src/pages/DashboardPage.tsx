...
const TITLES: Record<string, { title: string; sub: string }> = {
  overview: { title: 'Dashboard Overview',           sub: 'SAR Cycle 2024–25 · GAPC V4.0 · Electrical' },
  ...
  sar:      { title: 'SAR Progress',                  sub: 'Criteria score summary — marks achieved vs maximum' },
  docsearch:{ title: 'Document Search',               sub: 'Cross-document keyword search across all uploaded files' },
  reports:  { title: 'Reports & Export',              sub: 'Generate and preview SAR reports' },
  'sar-generator': { title: 'SAR Report Generator', sub: 'AI-powered NBA GAPC V4.0 Self-Assessment Report builder with scoring' },
  fdash:    { title: 'My Dashboard',                  sub: 'Your assigned criteria, tasks, and documents' },
  ...
  return (
    <div className="flex flex-col gap-4 max-w-screen-2xl mx-auto">
      ...
      <PageTransition panelKey={activePanel}>
        ...
        {activePanel === 'fdash'      && <FacultyDashboardPage />}
        ...
        {activePanel === 'sar'        && <SARProgressPage />}
        {activePanel === 'docsearch'  && <DocumentSearchPage />}
        {activePanel === 'reports'    && <ReportsPage />}
        {activePanel === 'settings'      && <SettingsPage />}
        {activePanel === 'sar-generator' && <SARGeneratorPage />}
