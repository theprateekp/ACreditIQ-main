...
import { CheckCircleIcon, WarningIcon, SparklesIcon } from '@/components/ui/Icon';
            ...
            <div key={gap.id} className="px-3 py-2.5" style={{ borderBottom: '1px solid var(--border)', backgroundColor: i % 2 === 0 ? 'var(--bg-surface)' : 'var(--bg-row-alt)' }}>
              ...
              <div className="flex items-center gap-2 flex-wrap ml-5">
                ...
                >
                  ...
                  {expandedGapId === gap.id ? 'Hide AI' : 'Ask AI'}
                </button>
              </div>
              ...
              {expandedGapId === gap.id && (
                <div className="ml-5 mt-2">
                  <AIQueryBox
                    compact
                    prefillPrompt={`For NBA GAPC V4.0 criterion ${gap.criterionId}, provide a detailed step-by-step action plan to fix this gap: "${gap.description}". Include specific documents needed, timeline, and how to maximize the ${gap.marksImpact} marks at risk.`}
                    placeholder={`Ask AI how to fix this ${gap.severity} gap…`}
                  />
                </div>
              )}
            </div>
