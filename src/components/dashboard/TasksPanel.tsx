...
import { ClockIcon, SparklesIcon } from '@/components/ui/Icon';
      ...
      <div className="card-header flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {showAI ? 'Hide AI' : 'AI Prioritize'}
          </button>
          <span className="text-xs font-normal" style={{ color: 'var(--text-muted)' }}>{(tasks ?? []).length} tasks</span>
        </div>
      </div>

      {/* AI prioritization box */}
      {showAI && (
        <div className="px-3 py-2" style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-row-alt)' }}>
          <AIQueryBox
            compact
            prefillPrompt={`I have ${overdueCount} overdue tasks and ${(tasks ?? []).length} total tasks for NBA GAPC V4.0 accreditation. The overdue tasks are: ${(tasks ?? []).filter(overdue).map(t => `"${t.title}" (${t.criterionId}, ${t.priority} priority)`).join('; ')}. Provide a prioritized action plan with specific steps to address these immediately.`}
            placeholder="Ask AI to prioritize and suggest actions for overdue tasks…"
          />
        </div>
      )}
