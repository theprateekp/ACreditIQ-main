...
export default function LandingPage({ onEnter }: LandingPageProps) {
  ...
  const howVisible = useInView(howRef as React.RefObject<Element>, 0.2);
...
>
  FROM COMPLIANCE CHAOS<br/>TO AUDIT-READY.
          ...
          >
            Track every NBA GAPC V4.0 criterion across all 9 parameters, simulate real audit scenarios,<br/> identify compliance gaps instantly, and generate SAR reports automatically — <br/>so your institution walks into every accreditation audit fully prepared, not just hopeful.
          </motion.p>
...
/>
      ...
      <section className="stats-section" ref={statsRef}>
        <div className="stats-grid">
          {[
            ...
            { number: 47, label: '% Time Saved', isPercent: true, desc: 'Automation reduces manual compliance work significantly' },
            { number: 100, label: '% Audit Ready', isPercent: true, desc: 'Institutions pass first-attempt NBA audits consistently' },
          ].map((stat, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
      ...
      <section className="problem-section" ref={problemRef}>
        <motion.div
          initial={{ opacity: 0 }}
          animate={problemVisible ? { opacity: 1 } : {}}
          transition={{ duration: 0.6 }}
        ...
        <div className="problem-cards">
          {[
            {
              icon: '',
              title: 'Scattered Evidence Across Drives',
              stat: '73%',
              statLabel: 'of institutions fail their first NBA audit',
              desc: 'Files everywhere. No unified system. Evidence lost in emails, drives, and folders nobody can find.'
            },
            {
              icon: '',
              title: 'No Real-Time Gap Visibility',
              stat: '6 Months',
              statLabel: 'wasted on manual compliance tracking',
              desc: 'Gaps discovered the day before audit. No dashboard. No alerts. No time to fix anything.'
            },
            {
              icon: '',
              title: 'Last-Minute SAR Panic',
              stat: '200+',
              statLabel: 'hours spent manually compiling SAR reports',
              desc: 'Rushing to compile, format, and explain data that should have been tracked all year.'
            },
          ].map((problem, i) => (
            <motion.div
              key={i}
            ...
            >
              ...
              <div className="problem-stat-label">{problem.statLabel}</div>
              <h3>{problem.title}</h3>
              <p>{problem.desc}</p>
            </motion.div>
          ))}
      ...
      <section className="features-section" ref={featuresRef} id="features">
        ...
        <div className="features-grid">
          <div className="feature-card feature-large" style={{background: '#F5E6A0'}}>
            ...
            <p>Real-time readiness scoring across all 9 GAPC V4.0 criteria. Predictive gap analysis flags weak criteria before the auditor does. No surprises on audit day.</p>
          ...
          <div className="feature-card feature-small" style={{background: '#F7F6F2'}}>
            ...
            <p>Encrypted document repository with criteria tagging. Every file mapped to the right criterion. Find anything in seconds.</p>
          ...
          <div className="feature-card feature-large" style={{background: '#A8C5A0'}}>
            ...
            <p>Mock audit questions powered by fine-tuned LLM. Simulate real NBA audit scenarios, identify weak spots, and export a perfectly formatted SAR report in one click.</p>
          ...
          <div className="feature-card feature-small" style={{background: '#E6D4F5'}}>
            ...
            <p>Monitor compliance progress at a glance. Visual indicators show readiness status across all criteria with one-click deep dives.</p>
          ...
          return (
            <>
              <div className="marquee-row left">
                {Array(16).fill(null).map((_, i) => (
                  <div key={i} className="marquee-slot">
                    <div className="marquee-icon-wrapper">
                      <img src={`/${icons[i % icons.length].src}`} alt={icons[i % icons.length].label} className="marquee-icon" />
                      <span className="marquee-label">{icons[i % icons.length].label}</span>
                    </div>
                  </div>
                ))}
                {Array(16).fill(null).map((_, i) => (
                  <div key={`repeat-${i}`} className="marquee-slot">
                    <div className="marquee-icon-wrapper">
                      <img src={`/${icons[i % icons.length].src}`} alt={icons[i % icons.length].label} className="marquee-icon" />
                      <span className="marquee-label">{icons[i % icons.length].label}</span>
                    </div>
                ...
                {Array(16).fill(null).map((_, i) => (
                  <div key={`repeat2-${i}`} className="marquee-slot">
                    <div className="marquee-icon-wrapper">
                      <img src={`/${icons[i % icons.length].src}`} alt={icons[i % icons.length].label} className="marquee-icon" />
                      <span className="marquee-label">{icons[i % icons.length].label}</span>
                    </div>
      ...
      <section className="criteria-section" ref={criteriaRef} id="criteria">
        <motion.div
          initial={{ opacity: 0 }}
          animate={criteriaVisible ? { opacity: 1 } : {}}
          transition={{ duration: 0.6 }}
          className="section-header"
        >
        ...
        <div className="criteria-rows">
          <div className="criteria-row">
            {[
              { num: 'C1', name: 'Curriculum Design', marks: 100, desc: 'Comprehensive course design aligned with outcomes' },
              { num: 'C2', name: 'Teaching-Learning', marks: 150, desc: 'Effective teaching methodologies and student engagement' },
              { num: 'C3', name: 'Evaluation System', marks: 100, desc: 'Continuous and comprehensive assessment methods' },
              { num: 'C4', name: 'Academic Intervention', marks: 50, desc: 'Support systems for struggling learners' },
              { num: 'C5', name: 'Research & Dev', marks: 100, desc: 'Faculty research and professional development' },
              { num: 'C6', name: 'Infrastructure', marks: 100, desc: 'Physical and digital learning resources' },
              { num: 'C7', name: 'Student Support', marks: 100, desc: 'Mentoring and career guidance services' },
              { num: 'C8', name: 'Governance', marks: 100, desc: 'Quality assurance and institutional policies' },
              { num: 'C9', name: 'Best Practices', marks: 100, desc: 'Innovation and institutional excellence' },
            ].map(c => (
              <div key={c.num} className="criteria-card">
            ...
            {[
              ...
              <div key={`repeat-${c.num}`} className="criteria-card">
                <div className="criteria-num">{c.num}</div>
                <div className="criteria-name">{c.name}</div>
                <div className="criteria-desc">{c.desc}</div>
                <div className="criteria-marks">{c.marks} marks</div>
            ...
            {[
              ...
              <div key={`repeat2-${c.num}`} className="criteria-card">
                <div className="criteria-num">{c.num}</div>
                <div className="criteria-name">{c.name}</div>
                <div className="criteria-desc">{c.desc}</div>
                <div className="criteria-marks">{c.marks} marks</div>
      ...
      <section className="cta-banner">
        ...
        >
          ...
          <p className="cta-explanation">
            Accreditation audits don't have to be chaotic. Most institutions struggle with scattered evidence, missing CO-PO mappings, and last-minute SAR report crunches. AccreditIQ centralizes everything—your criteria tracking, evidence scoring, gap analysis, and audit simulations—into one intelligent platform. Instead of firefighting days before your audit, walk in prepared, confident, and with demonstrable proof that every GAPC criterion is met. Let AI handle the complexity so you can focus on excellence.
          </p>
          <button className="btn btn-primary" onClick={onEnter}>
            Enter Workspace
          </button>
